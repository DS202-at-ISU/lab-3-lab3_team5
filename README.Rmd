...
avengers_died_at_least_once <- combined_filtered %>%
  filter(Reduce(`|`, across(starts_with("Death"), ~. == "YES")))

 #Calculate the percentage of Avengers who have died at least once
 percentage_died_at_least_once <- nrow(avengers_died_at_least_once) / nrow(combined_filtered) * 100
 
 percentage_died_at_least_once

# The statement is not true as the percentage of avengers that have died is 10%.
```
